...
func _ready() -> void:
	...
	soul_meter.value = 0
	p_2_current_health = health
	
	animation_player = get_tree().get_nodes_in_group("animationplayer")[1]
...
func _physics_process(delta: float) -> void:
	...
	if not moving and not attacking and not jumping and can_crouch and Input.is_action_pressed("crouch_p2"):
		crouch()
		crouching = true
	...
	# Handle jump.
	if Input.is_action_just_pressed("jump_p2") and is_on_floor() and not attacking:
...
#attack inputs
	if Input.is_action_just_pressed("A_p2") and not jumping and can_attack1 and not attacking:
		#maybe make an attacking timer for all attacks (recovery time)
		attacking = true
		animation_player.play("A1")
		attack_timer(0.75)

	if Input.is_action_just_pressed("B_p2") and not jumping and can_attack2 and not attacking:
		attacking = true
		animation_player.play("B1")
		attack_timer(.45)

	if can_special1 and Input.is_action_pressed("A_p2") and Input.is_action_just_pressed("B_p2") and not attacking:
		animation_player.play("RESET")
		attacking = true
		animation_player.play("S1")

	if can_special2 and Input.is_action_pressed("B_p2") and Input.is_action_just_pressed("C_p2") and not attacking:
		animation_player.play("RESET")
		attacking = true
		animation_player.play("S2")

	if can_special3 and Input.is_action_pressed("A_p2") and Input.is_action_just_pressed("C_p2") and not attacking:
		animation_player.play("RESET")
		attacking = true
...
func dash():
	if Input.is_action_just_pressed("left_p2") and jumping and can_dash:
		dashing = true
		can_dash = false
		await get_tree().create_timer(0.3).timeout
		dashing = false
	if Input.is_action_just_pressed("right_p2") and jumping and can_dash:
		dashing = true
		can_dash = false
		await get_tree().create_timer(0.3).timeout
		dashing = false
...
func roll_slots():
	...
	if Input.is_action_just_pressed("crouch_p2"):
		if combo_timer > 0:
			is_combo = true
			combo_timer = 0.0
		else:
			combo_timer = max_combo_delay
	if Input.is_action_just_pressed("A_p2"):
		if combo_timer > 0:
			is_combo = true
			combo_timer = 0.0
		else:
	...
	if is_combo and Input.is_action_pressed("crouch_p2") and Input.is_action_just_pressed("A_p2") and can_use_slot == true:
		...
		if GlobalData.p2_character == "Anathema":
			...
			#$player2_LOADEDANIMS/Sprite2D/FrameData.damage = special_damage
...
func special_attack_assign():
	if special_attack == 0:
		...
		print("canS1")
		print(can_special1)
		$player2_LOADEDANIMS/Sprite2D/FrameDataSpecial1.damage = special_damage
	if special_attack == 1:
		can_special2 = true
		print("canS2")
		print(can_special2)
		$player2_LOADEDANIMS/Sprite2D/FrameDataSpecial2.damage = special_damage
	if special_attack == 2:
		can_special3 = true
		print("canS3")
		print(can_special3)
		$player2_LOADEDANIMS/Sprite2D/FrameDataSpecial3.damage = special_damage

func insert_player_anims():
	if GlobalData.p2_character == "Anathema":
		#if button pressed then insert specific character anims
		var anathema_instance = anathema.instantiate()
		$player2_LOADEDANIMS.add_child(anathema_instance)
		#grab the animation hitboxes
		anathema_instance.get_node("FrameData").collision_layer = 2
		anathema_instance.get_node("FrameData2").collision_layer = 2
		anathema_instance.get_node("FrameDataSpecial1").collision_layer = 2
		anathema_instance.get_node("FrameDataSpecial2").collision_layer = 2
		anathema_instance.get_node("FrameDataSpecial3").collision_layer = 2
		get_node("Hurtbox").collision_mask = 4
	if GlobalData.p2_character == "Talisman":
		#if button pressed then insert specific character anims
		var talisman_instance = talisman.instantiate()
		$player2_LOADEDANIMS.add_child(talisman_instance)
		#grab the animation hitboxes
		talisman_instance.get_node("FrameData").collision_layer = 2
		talisman_instance.get_node("FrameData2").collision_layer = 2
		talisman_instance.get_node("FrameDataSpecial1").collision_layer = 2
		talisman_instance.get_node("FrameDataSpecial2").collision_layer = 2
		talisman_instance.get_node("FrameDataSpecial3").collision_layer = 2
		get_node("Hurtbox").collision_mask = 4

func insert_player_stocks():
	#if charactar selction button pressed 
	if GlobalData.p2_character == "Anathema":
...
func send_damage(amount: int) -> void:
	if crouching:
		...
		p_1_combo_timer.start()
		
		print("Damage: ", amount)
		print("p_2current_health: ", + p_2_current_health)
		#on hit change color for now
		$player2_LOADEDANIMS/Sprite2D.self_modulate = Color.RED
		await get_tree().create_timer(.5).timeout
		$player2_LOADEDANIMS/Sprite2D.self_modulate = Color.WHITE
	
	health_bar.set_health(p_2_current_health)
	await get_tree().create_timer(0.01).timeout
	life_check()


func life_check():
	if (p_2_current_health <= 0):
		lives_lost += 1
		stocks.get_child(0).queue_free()
		#reset round
		p_2_current_health = 100
...
func lose():
	#var t = round_text.instantiate()
	#t.text = "Player 1 Wins"
	#text_marker.add_child(t)
	await get_tree().create_timer(1.5).timeout
	get_tree().paused = true
	text_animation_player.play("player_1_wins")
...
func anim_done(name: String):
	...
	if name == "S1":
		attacking = false
		can_special1 = false
		$player2_LOADEDANIMS/Sprite2D/FrameDataSpecial1.damage = 0
	if name == "S2":
		attacking = false
		can_special2 = false
		$player2_LOADEDANIMS/Sprite2D/FrameDataSpecial2.damage = 0
	if name == "S3":
		attacking = false
		can_special3 = false
		$player2_LOADEDANIMS/Sprite2D/FrameDataSpecial3.damage = 0
	elif name == "jump":
		jumping = false

func _on_p_1_combo_timer_timeout() -> void:
	#send combo information to the sticker manager 
	if hits_in_a_row == 2:
		sticker_manager.add_sticker_p_1("two")
		print("sticker change to 2")
			#hits_in_a_row = 0
	elif hits_in_a_row == 3:
		sticker_manager.add_sticker_p_1("three")
		print("sticker chnage to 3")
			#hits_in_a_row = 0
	elif hits_in_a_row == 4:
		sticker_manager.add_sticker_p_1("four")
	elif hits_in_a_row == 5:
		sticker_manager.add_sticker_p_1("five")
